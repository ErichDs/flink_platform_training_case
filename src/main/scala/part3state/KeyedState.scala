...
object KeyedState {
  ...
  def demoFirstKeyedState(): Unit = {
    ...
    val numEventsPerUserNaive = eventsPerUser.process(
      new KeyedProcessFunction[String, ShoppingCartEvent, String] { // Key, event, result // instantiated ONCE PER KEY
    ...
    val allEventsPerUserStream = eventsPerUser.process(
      new KeyedProcessFunction[String, ShoppingCartEvent, String] {
        ...
        override def processElement(
          ...
          // Scala 2.13 & Scala 3
          // import scala.jdl.CollectionConverters._

          val currentEvents: Iterable[ShoppingCartEvent] = stateEventsForUser
            .get()   // does not return a plain List, but a Java Iterable
