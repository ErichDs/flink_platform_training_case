...
object RichFunctions {
  ...
  val tenxNumbersWithLifeCycle: DataStream[Int] = numbersStream.map(
  ...
  val tenxNumbersWithLifeCycle_2: DataStream[Int] = numbersStream.map(
    new RichMapFunction[Int, Int] {
      override def map(value: Int) = value * 10 // mandatory override
      ...
      // called BEFORE data goes through
      override def open(parameters: Configuration): Unit =
        println("Starting my second work!")

      // invoked AFTER all the data
      override def close(): Unit =
        println("Finishing my second work...")
    }
  )
  ...
  def exercise(): Unit = {
    ...
    val itemsPurchasedStream_v2: DataStream[String] =
      shoppingCartStream.flatMap(
        new FlatMapFunction[AddToShoppingCartEvent, String] {
    ...
    val itemsPurchasedStream_v4: DataStream[String] =
      shoppingCartStream.process(
        new ProcessFunction[AddToShoppingCartEvent, String] {
          override def processElement(
            event: AddToShoppingCartEvent,
