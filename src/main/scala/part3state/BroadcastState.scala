...
object BroadcastState {
  ...
  // issue a warning if a quantity > threshold
  ...
  def changingThresholds(): Unit = {
    val thresholds: DataStream[Int] = env.addSource(
    ...
    val notificationsStream = eventsByUser
      ...
      .process(
        new KeyedBroadcastProcessFunction[String, ShoppingCartEvent, Int, String] { // key, first event, broadcast, output
            ...
            event match {
              case AddToShoppingCartEvent(userId, sku, quantity, time) =>
                ...
                if (quantity > currentThreshold)
                  out.collect(
                    s"User $userId attempting to purchase $quantity items of $sku when threshold is $currentThreshold"
                  )
              case _ =>
            }
