...
object HandlingMultipleStreams {
  ...
  def demoWindowJoins(): Unit = {
    ...
    val joinedStream = shoppingCartEvents
      ...
      .where(shoppingCartEvents => shoppingCartEvents.userId)
  ...
  // Interval Joins = correlation between events A nad B if durationMin < timeA - timeB < durationMax
  // involves EVENT TIME
  ...
  def demoIntervalJoins(): Unit = {
    ...
    // we need to extract event time from both streams
    ...
    val intervalJoinedStream = shoppingCartEvents
      ...
      .lowerBoundExclusive() // interval is by default inclusive (optional)
      .upperBoundExclusive()
      .process(
        new ProcessJoinFunction[ShoppingCartEvent, CatalogEvent, String] {
  ...
  def demoConnect(): Unit = {
    ...
    // variables - will use single-threaded (just for the sake of demonstration)
