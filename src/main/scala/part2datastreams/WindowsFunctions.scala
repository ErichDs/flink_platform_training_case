...
object WindowsFunctions {
  ...
  val events: List[ServerEvent] = List(
    bob.register(
      2.seconds
    ), // player "Bob" registered 2s after the server started
  ...
  val eventStream: DataStream[ServerEvent] = env
    ...
    .assignTimestampsAndWatermarks(
      // extract timestamps for events (event time) + watermarks
      WatermarkStrategy
  ...
  def demoCountByWindow_v3(): Unit = {
    val registrationsPerThreeSeconds: DataStream[Long] =
      threeSecondsTumblingWindow.aggregate(new CountByWindowV3)
    registrationsPerThreeSeconds.print()
    env.execute()
  }
  ...
  def demoCountByTypeByWindow_v2(): Unit = {
    val finalStream =
      threeSecondsTumblingWindowByType.process(new CountByWindowV2)
    finalStream.print()
    env.execute()
...
^|------------ window one ----------- +
  ...
  /** Session Windows
    * Groups of events with NO MORE THAN a certain time gap in between them
    */
  ...
  class CountByGlobalWindowAll
      extends AllWindowFunction[ServerEvent, String, GlobalWindow] {
    //                          ^ input     ^ output    ^ window type
    override def apply(
        window: GlobalWindow,
        input: Iterable[ServerEvent],
      ...
      val registrationEventCount =
        input.count(event => event.isInstanceOf[PlayerRegistered])
      out.collect(
        s"Window [$window] $registrationEventCount"
      )
    }
  }
  ...
  /** Exercise: what was the time window (continuous 2s) when we had THE MOST registration events?
    *  - what kind of windows functions should we use? ALL WINDOW FUNCTION
    *  - what kind of windows should we use? SLIDING WINDOWS
    */

  class KeepWindowsAndCountFunction
  ...
  def windowFunctionsExercise(): Unit = {
    val slidingWindows: DataStream[(TimeWindow, Long)] = eventStream
      ...
      .apply(new KeepWindowsAndCountFunction)

    val localWindows: List[(TimeWindow, Long)] =
      slidingWindows.executeAndCollect().toList
    val bestWindow: (TimeWindow, Long) = localWindows.maxBy(_._2)

    print(
      s"The best window is ${bestWindow._1} with ${bestWindow._2} registration events."
    )
  }
